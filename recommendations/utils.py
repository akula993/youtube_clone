...
class RecommendationEngine:
    ...
    def update_recommendations(self):
        ...
        recommendations = self.get_recommendations(50)

        for i, video in enumerate(recommendations):
            score = 1.0 - (i * 0.02)  # Убывающий рейтинг
            reason = self.get_recommendation_reason(video)
    ...
    def get_recommendation_reason(self, video):
        """Определить причину рекомендации"""
        if self.user.subscribed_to.filter(id=video.uploader.id).exists():
            return "Новое видео от подписки"
        elif self.user.liked_videos.filter(categories__in=video.categories.all()).exists():
            return "Похоже на понравившиеся"
        elif WatchHistory.objects.filter(user=self.user, video__categories__in=video.categories.all()).exists():
            return "Основано на истории просмотров"
        else:
            return "Популярное видео"
